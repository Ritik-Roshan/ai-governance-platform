...
- ✅ **Multi-Sectoral Analysis** - Merges disparate datasets (e.g., health, crime, infrastructure) into a unified view, allowing the RAG and ML models to identify complex, cross-domain risks.
---
...
Before running the main app, you can test your Gemini API key with a built-in utility endpoint. After starting the server:

**Visit:** `http://localhost:5000/api/test/gemini`

You should see a JSON response with `{"status": "success"}`. If you see an error, double-check your `.env` file and restart the server.
...
### Advanced RAG Indexing Strategy

This platform does not just "dump" files into a vector store. It uses an intelligent, multi-step indexing strategy on startup (`initialize_platform`) to ensure high-quality, relevant answers from the AI Assistant:

1.  **Dataset Summaries:** A high-level summary document is created for each dataset, giving the RAG context of what data is available (e.g., column names, record count).
2.  **High-Precision Record Indexing:** For critical datasets (like hospitals), each *individual record* (e.g., each hospital) is indexed as a separate, detailed document. This allows the RAG to fetch specific, entity-level details (e.g., "Tell me about Sassoon Hospital").
3.  **Intelligent Sampling:** For very large datasets, a random sample of records is indexed. This prevents the vector store from being flooded with thousands of noisy or repetitive documents, improving the relevance of search results.

This hybrid approach allows the AI Assistant to answer both broad questions ("What data do you have on infrastructure?") and highly specific questions ("What are the details for hospital X?").

### Dynamic "Train-or-Load" Model System

The platform is built for robust, persistent operation. On startup, the `initialize_platform` function:

1.  **Attempts to Load:** It first searches the `models/saved_models/` directory for pre-trained model files (e.g., `.pkl`).
2.  **Trains if Missing:** If any model file is *not* found (or fails to load), the system automatically triggers the `ModelTrainer` module.
3.  **Saves New Models:** The `ModelTrainer` trains the missing models from scratch using the processed data and saves the new model files for all future runs.

This self-healing "train-on-first-run" architecture ensures the platform is always ready, even in a fresh deployment, without requiring a separate, manual training step.
